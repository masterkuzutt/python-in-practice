...
def main():
    ...
    # ただそれだけ。
    # factory class をラップする抽象的なメソッド（ceate_diagram())がいるだけ。
    # DiagramFactory とかsvnDiagramFactoryはFactory Method.
    # ⇒ textとかDiagramとか作ってる
    # テストしやすいとか言っているサイトがあった気がするがなぜか？
    # ⇒同じ関数のテストを擦れればよいから。
    # 　さらにここではわからないが、dirgram2.pyでの改善案では、クラス内の
    # 　make_diagramとかを抽象クラス側の実装から変えなくてもいいような
    # 　になるような修正をしている。
